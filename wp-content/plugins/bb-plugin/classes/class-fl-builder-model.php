	...
	static public function get_uncategorized_modules( $show_disabled = false ) {
		...
		if ( in_array( 'widget', $enabled_modules ) ) {
			foreach ( $widgets as $widget ) {
				...
				$module->isAlias = false; // @codingStandardsIgnoreLine
				$widget->description = isset( $widget->widget_options['description'] ) ? $widget->widget_options['description'] : '';
				$widget->group = array( 'wordpresswidgets' );

				if ( ! isset( $widget->icon ) ) {
					$widget->icon = FLBuilderModule::get_widget_icon();
				}

				unset( $widget->alt_option_name );
				unset( $widget->control_options );
				unset( $widget->id_base );
				unset( $widget->number );
				unset( $widget->option_name );
				unset( $widget->updated );
