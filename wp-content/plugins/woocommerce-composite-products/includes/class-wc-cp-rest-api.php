		...
		if ( $product_id && 'composite' === $product_type ) {
			switch ( $field_name ) {
				...
				case 'composite_components' :
					...
					if ( ! WC_CP_Core_Compatibility::is_wc_version_gte_2_7() ) {
						throw new WC_REST_Exception( 'woocommerce_rest_unsupported_composite_field', __( 'WooCommerce 2.7+ is required to create/update Composite product components.', 'woocommerce-composite-products' ), 400 );
					}
				...
				case 'composite_scenarios' :
					...
					// Updating shouldn't be registered for WC < 2.7, but oh well.
					if ( ! WC_CP_Core_Compatibility::is_wc_version_gte_2_7() ) {
						throw new WC_REST_Exception( 'woocommerce_rest_unsupported_composite_field', __( 'WooCommerce 2.7+ is required to create/update Composite product scenarios.', 'woocommerce-composite-products' ), 400 );
					}
	...
	public static function sanitize_rest_api_component_data( $component_data ) {
		...
		if ( ! empty( $component_data ) ) {
			foreach ( $component_data as $key => $value ) {
				...
				if ( in_array( $key, array( 'title', 'description', 'query_type', 'options_style', 'thumbnail_src' ) ) ) {
