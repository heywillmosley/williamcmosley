...
function wpaal_add_affiliate_links($content) {   
	...
	//	}
		...
		foreach($allkeys as $ident => $key) { { { 
				...
				if($aal_apirequestno < 5 ) if($sofar<$notimes && $querylimit!='overquota' && $apikey &&($clickbankactive || $amazonactive || $shareasaleactive || $cjactive || $ebayactive || $bestbuyactive || $walmartactive || $envatoactive || $rakutenactive)) {
